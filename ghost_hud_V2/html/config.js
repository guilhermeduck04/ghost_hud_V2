// CORREÇÃO: Removida a linha "let currentConfig = {};" para evitar a redeclaração.
...
function initializeConfigEvents() {
    // CORREÇÃO: Adicionado o evento de clique para o botão fechar, que antes estava no script.js.
...
function buildConfigInterface() {
    ...
    addConfigToggle('minimapEnabled', 'Minimapa', 'map-marker-alt', currentConfig.minimapEnabled); // Nome corrigido para minimapEnabled
    ...
    const elements = [
        ...
        {id: 'radiation', name: 'Radiação', icon: 'radiation'},
        {id: 'temperature', name: 'Temperatura', icon: 'temperature-high'},
        ...
        {id: 'id', name: 'ID do Jogador', icon: 'id-card'},
...
function addConfigToggle(id, label, icon, isChecked, isElement = false) {
    ...
    input.addEventListener('change', () => {
        ...
        SaveConfig();
        UpdateHUDPosition(currentConfig); // Atualiza a HUD em tempo real
    });
...
// A função UpdateHUDPosition foi movida para script.js para evitar duplicação,
// mas o painel de config ainda precisa chamá-la. Como ambos os scripts
// compartilham o escopo global, a função definida em script.js será usada.
